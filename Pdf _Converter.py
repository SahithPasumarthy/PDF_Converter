...
def get_img_func():
    root.img_file_name = tkfileDialog.askopenfilename(initialdir="/", title="Select file", filetypes=(
...
def convert():
    ...
    root.location = tkfileDialog.asksaveasfilename(
    ...
    print(name)
    pdfconvert(loc, name)
    messagebox.showinfo('Message', 'Your PDF file ' +
                        tempname+' is Saved Successfully')
...
def get_Split_file_func():
    root.get_split_filename = tkfileDialog.askopenfilename(
        initialdir="/", title="Select file", filetypes=(("PDF files", "*.PDF"), ("all files", "*.*")))
...
def get_first_pdf_fn():
    root.first_pdf_file = tkfileDialog.askopenfilename(
        initialdir="/", title="Select file", filetypes=(("PDF files", "*.PDF"), ("all files", "*.*")))
...
def get_second_pdf_fn():
    root.second_pdf_file = tkfileDialog.askopenfilename(
        initialdir="/", title="Select file", filetypes=(("PDF files", "*.PDF"), ("all files", "*.*")))
...
def merge():
    ...
    messagebox.showinfo('Message', 'Your PDF file '+p1 +
                        ' and '+p2+' is Merged Successfully')
...
root.geometry('420x491')
image1 = Image.open(r"C:\Users\dell\Downloads\pdf.png")
...
option = IntVar()
root.title("PDF Converter by Sahith")
x = Radiobutton(root, bg='#ffffff', text="Convert", variable=option,
                value=1, command=select1, padx=30, pady=50).grid(column=0, row=0)
y = Radiobutton(root, bg='#ffffff', text="Split", variable=option,
                value=2, command=select2, padx=30, pady=50).grid(column=1, row=0)
z = Radiobutton(root, bg='#ffffff', text="Join", variable=option,
                value=3, command=select3, padx=30, pady=50).grid(column=2, row=0)
...
actionBtn = ttk.Button(root, text="Reset", width=15,
                       command=reset_btn_fn).place(x=155, y=440)

get_file_button = ttk.Button(
    f1, text="Load Image", width=25, command=get_img_func)
get_file_button.grid(row=4, column=1)
convert_button = ttk.Button(
    f1, text="Convert to PDF", width=25, command=convert)
convert_button.grid(row=1, column=1)

split_file_get_button = ttk.Button(
    f2, text="Load PDF", width=25, command=get_Split_file_func)
...
get_first_pdf_btn = ttk.Button(
    f3, text="Load PDF 1", width=25, command=get_first_pdf_fn)
get_first_pdf_btn.grid(row=0, column=1)
get_second_pdf_btn = ttk.Button(
    f3, text="Load PDF 2", width=25, command=get_second_pdf_fn)
